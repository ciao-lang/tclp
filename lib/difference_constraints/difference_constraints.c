...
{  
  ...
  intmach_t id = new_diff_var_space(w,space);
...
{  
  ...
  intmach_t *vars = (intmach_t*) malloc (space->size * sizeof(intmach_t));
  intmach_t i;
  intmach_t *sp;
...
CBOOL__PROTO(initial_space_c)
{
  ...
  printf("\nInitial Space\n"); fflush(stdout);
#endif
  space = create_space(w);
  
  atom_incr_dc_num_vars = 
    GET_ATOM("difference_constraints_rt_ll:$incr_dc_num_vars");
  atom_decr_dc_num_vars = 
    GET_ATOM("difference_constraints_rt_ll:$decr_dc_num_vars");
  functor_forward_trail = 
    SetArity(GET_ATOM("forward_trail:$forward_trail"), 2);
  functor_put_dc_value = 
    SetArity(GET_ATOM("difference_constraints_rt_ll:$put_dc_value"), 3);
  functor_put_dc_pi = 
    SetArity(GET_ATOM("difference_constraints_rt_ll:$put_dc_pi"), 1);
  functor_dbm_id = SetArity(GET_ATOM("$dbm_id"), 2);
  functor_put_dc_space = 
    SetArity(GET_ATOM("difference_constraints_rt_ll:$put_dc_space"), 1);
