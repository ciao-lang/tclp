...
expand_command(aggregate(_Preds), []) :- !,
    print('\nWarning: the declaration aggregate/1 is not supported, use agg_entail/1 or agg_join/1 instead.\n').
expand_command(agg_entail(Preds), Clauses) :- !,
    expand_command_table(Preds, Clauses, [], entail).
expand_command(agg_join(Preds), Clauses) :- !,
...
add_aggregate_translation_(entail,[M|Modes],[M/2|CompModes],Trans0) :-
    ...
    M =.. AggMode,
    append(AggMode,[B,A],AggEntailCall),                 % Agg's paramenters at the beginning
    EntailCall =.. AggEntailCall,
...
add_aggregate_translation_(join,[M|Modes],[M/3|CompModes],Trans0) :-
    ...
    M =.. AggMode,
    append(AggMode, [A,B,C], AggJoinCall),             % Agg's paramenters at the beginning   
    JoinCall =.. AggJoinCall,
    (
        'trans$entail_agg'(M) ->
        Trans0 = [
