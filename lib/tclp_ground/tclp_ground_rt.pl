...
call_entail_((Agg/_,A), (Agg/_,B))            :- ground_entail(Agg,A,B).
answer_compare_((Agg/_,A), (Agg/_,B),1)    :- ground_compare(Agg,A,B,1),!.
answer_compare_((Agg/_,A), (Agg/_,B),-1)    :- ground_compare(Agg,A,B,-1),!.
answer_compare_((Agg/_,A), (Agg/_,B),agg((Agg/_,New))) :- ground_join(Agg,A,B,New).

% apply_answer_(V, (Agg,B))     :- get(V,(Agg,A)), \+ ground(A), !, A = B.
% apply_answer_(V, (Agg/_,B))     :- get(V,(Agg/_,A)), ground(A), ground_compare(Agg,A,B,1).
apply_answer_(V, (Agg/_,B)) :- get(V,(Agg/_,A)), ground_apply(Agg,A,B).